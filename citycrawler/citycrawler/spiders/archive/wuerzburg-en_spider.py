...
class WuerzburgSpider(scrapy.Spider):
    name = "wuerzburg-en"
    allowed_domains = ['wuerzburg.de']
    allowed_url_path = "wuerzburg.de/en"
    start_urls = ['https://www.wuerzburg.de/en']
    # custom_settings = {
    ...
    def parse(self, response):
        ...
        if type(response) is scrapy.http.response.html.HtmlResponse:
            ...
            for href in response.css('a::attr(href)').getall():
                if self.check_href(response.urljoin(href)):
    ...
    def check_href(self, href):
        ...
        return (parsed_link.scheme == 'http' or parsed_link.scheme == 'https' or (not parsed_link.scheme and parsed_link.path)) and self.allowed_url_path in href
