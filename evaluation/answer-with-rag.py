...
def get_elasticsearch_documents(es_retriever, query, top_k=1, probability_border=0.6, filter=False):
    result = []
...
def generate_answers(source_path, target_path, generator, es_retriever, flags=""):
    source_content = ''
    ...
    with jsonlines.open(target_path, "a") as target_file:
        ...
        for line in tqdm(source_file, initial=num_target_lines, total=len(source_file) + num_target_lines):
            ...
            candidates = generator.predict(query=line_json['query'], documents=documents, top_k=1)
            line_json['generated_answer'] = candidates['answers'][0]['answer']
...
def main(args):
    ...
    generator = RAGenerator(embed_title=False, retriever=retriever)

    generate_answers(source_path, target_path, generator, retriever, flags)
